...
internal sealed class LinuxLogindPowerManagement : IPowerManagement
{
    ...
    public void BlockSleep()
    {
        ...
        try
        {
            ...
            if (msg == nint.Zero)
            {
                throw new OutOfMemoryException("Could not construct message");
            }
            ...
            {
                throw new OutOfMemoryException("Could not append args to message");
            }
            ...
            if (reply == nint.Zero)
            {
                        ...
                        in error, LinuxDbusApi.DbusErrorInteractiveAuthorizationRequired))
                {
                    throw new UnauthorizedAccessException(
                        "Preventing sleep requires authentication on this system. See https://github.com/jonschz/jellyfin-plugin-preventsleep/blob/main/docs/Linux.md");
                }
            ...
            if (!LinuxDbusApi.dbus_message_iter_init(reply, out var replyIter) ||
                LinuxDbusApi.dbus_message_iter_get_arg_type(in replyIter) != LinuxDbusApi.DbusTypeUnixFd)
            {
